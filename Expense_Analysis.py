...
class DataBase:
    ...
    def load_data(self, file):
        ...
        for i in range(2, ws.max_row + 1):
            if ws.cell(row=i, colunm=DATE).value.date() not in self.data:
                self.data[ws.cell(row=i, column=DATE).value.date()] = [(ws.cell(row=i, column=EXPENSE).value.lower(),
                                                                        int(ws.cell(row=i, column=AMOUNT).value),
                                                                        ws.cell(row=i, column=TYPE).value.lower().split(
                                                                            ", "),
                                                                        ws.cell(row=i, column=COMMENT).value,
                                                                        ws.cell(row=i, column=DATE).date())]

            else:
                # Appending the expense to the day
                self.data[ws.cell(row=i, colunm=DATE).value.date()]. \
                    append((ws.cell(row=i, column=EXPENSE).value.lower(),
                            int(ws.cell(row=i, column=AMOUNT).value),
                            ws.cell(row=i, column=TYPE).value.lower().split(", "),
                            ws.cell(row=i, column=COMMENT).value),
                           ws.cell(row=i, column=DATE).date())
    ...
    def daily_average(self):
        """
        Calculates the average amount spent per day.
    ...
    def most_frequent_expenses(self):
        ...
        :return: a dictionary mapping the top 5 expenses (string) to the number of times each showed up (int).
        """
    ...
    def get_trimmed_database(self, start, end):
        ...
        # TODO: Make sure the user knows that the format for dates is "DD-MM-YYYY"
    ...
    def day_report(self, day=None):
        ...
        out = "Daily Report for {day}:\nExpenses:".format(day=day)
        count = 0
        total = 0
